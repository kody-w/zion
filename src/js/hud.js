(function(exports) {
  ...
  function initHUD(container) {
    ...
    // Nearby players list (right side, below minimap)
    ...
    nearbyPlayersList.style.cssText = `
      ...
      font-size: 11px;
    `;
    nearbyPlayersList.innerHTML = '<div style="font-weight:600;margin-bottom:4px;color:#8890a0;font-size:11px;">Nearby Players</div>';
    hudOverlay.appendChild(nearbyPlayersList);

    // Federation status panel (bottom-right, subtle)
    ...
    federationPanel.style.cssText = `
      ...
      backdrop-filter: blur(4px);
    `;
    federationPanel.innerHTML = '<div style="font-weight:600;margin-bottom:4px;color:#8888cc;font-size:11px;">Federation</div>';
  ...
  function initToolbar() {
    ...
    actions.forEach(function(action) {
      ...
      btn.style.cssText = 'background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.15);border-radius:8px;padding:6px 10px;text-align:center;font-size:10px;min-width:48px;cursor:pointer;transition:background 0.15s;';
      btn.innerHTML = '<div style="font-weight:bold;color:' + action.color + ';font-size:13px;letter-spacing:0.5px;">' + action.key + '</div><div style="color:#999;margin-top:1px;font-size:9px;">' + action.label + '</div>';
      btn.onmouseenter = function() { this.style.background = 'rgba(255,255,255,0.12)'; };
      btn.onmouseleave = function() { this.style.background = 'rgba(255,255,255,0.06)'; };
    ...
    coordsEl.style.cssText = 'position:absolute;bottom:250px;left:20px;background:rgba(5,8,18,0.5);border-radius:6px;padding:3px 8px;font-size:10px;color:#556;font-family:monospace;pointer-events:none;';
    hud.appendChild(coordsEl);

    // Weather and time display (right side, below nearby players)
    var infoPanel = document.createElement('div');
    infoPanel.style.cssText = 'position:absolute;top:250px;right:20px;background:rgba(5,8,18,0.8);border-radius:8px;padding:8px 12px;min-width:120px;font-size:12px;pointer-events:none;backdrop-filter:blur(4px);border:1px solid rgba(255,255,255,0.08);';
  ...
  function showNPCDialog(npcData) {
    ...
    npcDialogEl.style.cssText = 'position:absolute;bottom:60px;left:50%;transform:translateX(-50%);' +
      'background:rgba(5,8,18,0.92);border:1px solid ' + color + '44;border-radius:14px;' +
      'padding:16px 20px;min-width:370px;max-width:460px;pointer-events:auto;' +
      'box-shadow:0 4px 24px rgba(0,0,0,0.6);backdrop-filter:blur(12px);animation:slideIn 0.3s ease-out;';
  ...
  function initQuestTracker() {
    ...
    questTrackerEl.style.cssText = 'position:absolute;top:320px;right:20px;background:rgba(10,14,26,0.9);' +
      'border-left:3px solid #d4af37;border-radius:8px;padding:12px 14px;min-width:230px;max-width:280px;' +
      'font-size:12px;pointer-events:auto;max-height:200px;overflow-y:auto;' +
      'box-shadow:0 2px 12px rgba(0,0,0,0.4);backdrop-filter:blur(4px);';
    questTrackerEl.innerHTML = '<div style="font-weight:bold;color:#d4af37;margin-bottom:6px;font-size:13px;letter-spacing:0.5px;">📋 Active Quests</div>';
  ...
  function createTutorialTooltip() {
    ...
    styleSheet.textContent = `
      @keyframes tutorialPulse {
        0%, 100% {
          box-shadow: 0 4px 20px rgba(0, 0, 0, 0.5), 0 0 8px rgba(218, 165, 32, 0.15);
        }
        50% {
          box-shadow: 0 4px 20px rgba(0, 0, 0, 0.5), 0 0 16px rgba(218, 165, 32, 0.35);
        }
      }
    `;
  ...
  function renderTutorialStep() {
    ...
    tutorialEl.style.cssText = 'position:fixed;bottom:30px;left:50%;transform:translateX(-50%);' +
      'background:rgba(5,8,18,0.92);border:1px solid rgba(218,165,32,0.3);border-radius:14px;' +
      'padding:20px 28px;width:420px;pointer-events:auto;z-index:500;' +
      'box-shadow:0 8px 32px rgba(0,0,0,0.6);color:#E8E0D8;font-family:Arial,sans-serif;' +
