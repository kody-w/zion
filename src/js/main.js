(function(exports) {
  ...
  async function init() {
    ...
    // PlayerOnboarding: init onboarding for new players, record return for existing
    if (PlayerOnboarding && playerOnboardingState) {
      if (PlayerOnboarding.initOnboarding && !PlayerOnboarding.isOnboardingComplete(playerOnboardingState, username)) {
        PlayerOnboarding.initOnboarding(playerOnboardingState, username, worldTime);
      }
      if (PlayerOnboarding.recordReturn) {
        PlayerOnboarding.recordReturn(playerOnboardingState, username, worldTime);
      }
    }

    // MultiverseReputation: init player in reputation system
    if (MultiverseReputation && MultiverseReputation.initPlayer && multiverseRepState) {
      MultiverseReputation.initPlayer(multiverseRepState, username, 'zion');
    }

    // WarmthSystem: record daily login
    if (WarmthSystem && WarmthSystem.recordDailyLogin && warmthState) {
      WarmthSystem.recordDailyLogin(warmthState, username, worldTime);
    }

    // RecoverySystem: check if player needs recovery assistance
    if (RecoverySystem && RecoverySystem.checkCondition && recoveryState && economyLedger) {
      var playerBalance = Economy && Economy.getBalance ? Economy.getBalance(economyLedger, username) : 0;
      var condition = RecoverySystem.checkCondition(recoveryState, username, { spark: playerBalance });
      if (condition && condition.needsRecovery && RecoverySystem.enterRecovery) {
        RecoverySystem.enterRecovery(recoveryState, username, condition.conditionId, worldTime);
      }
    }
  ...
  function handleTradeProtocolMessage(msg) {
    ...
    switch (result.type) {
      ...
      case 'trade_complete':
        ...
        // SkillMastery: trading XP
        if (SkillMastery && SkillMastery.addXP && skillMasteryState) {
          SkillMastery.addXP(skillMasteryState, 'trading', 10);
        }

        // MultiverseReputation: earn rep from trade
        if (MultiverseReputation && MultiverseReputation.earnReputation && multiverseRepState) {
          MultiverseReputation.earnReputation(multiverseRepState, localPlayer.id, 'zion', 3, 'trade');
        }
        ...
        if (EconomySimulator && EconomySimulator.createSnapshot && economySimState && marketDynamicsState) {
          economySimState.lastSnapshot = EconomySimulator.createSnapshot(marketDynamicsState);
        }
  ...
  function handleResourceHarvest(node) {
    ...
    if (result.success) {
      ...
      // AsyncCollab: contribute gathering effort to active projects
      if (AsyncCollab && AsyncCollab.contribute && asyncCollabState && localPlayer) {
        var myProjects = AsyncCollab.getPlayerProjects ? AsyncCollab.getPlayerProjects(asyncCollabState, localPlayer.id) : [];
        if (myProjects.length > 0) {
          AsyncCollab.contribute(asyncCollabState, localPlayer.id, myProjects[0].id, 1, 'Gathered ' + itemId, worldTime);
        }
      }

      // PlayerOnboarding: complete harvest step
      if (PlayerOnboarding && PlayerOnboarding.completeStep && playerOnboardingState) {
        PlayerOnboarding.completeStep(playerOnboardingState, localPlayer.id, 'harvest_item', worldTime);
      }
  ...
  function handleCraft(recipeId) {
    ...
    if (result.success) {
      ...
      // AsyncCollab: contribute crafting effort to active projects
      if (AsyncCollab && AsyncCollab.contribute && asyncCollabState && localPlayer) {
        var craftProjects = AsyncCollab.getPlayerProjects ? AsyncCollab.getPlayerProjects(asyncCollabState, localPlayer.id) : [];
        if (craftProjects.length > 0) {
          AsyncCollab.contribute(asyncCollabState, localPlayer.id, craftProjects[0].id, 2, 'Crafted ' + recipeId, worldTime);
        }
      }

      // PlayerOnboarding: complete craft step
      if (PlayerOnboarding && PlayerOnboarding.completeStep && playerOnboardingState) {
        PlayerOnboarding.completeStep(playerOnboardingState, localPlayer.id, 'craft_item', worldTime);
      }
    ...
    if (data.mode !== undefined) {
      ...
      if (World && World.confirmPlacement && sceneContext && localPlayer) {
        ...
        if (result && result.error) {
          ...
          if (SkillMastery && SkillMastery.addXP && skillMasteryState) {
            SkillMastery.addXP(skillMasteryState, 'building', 10);
          }

          // WorldShaper: update zone aesthetic from new build
          if (WorldShaper && WorldShaper.updateAesthetic && worldShaperState) {
            WorldShaper.updateAesthetic(worldShaperState, currentZone, worldTime);
          }

          // AsyncCollab: building effort contributes to projects
          if (AsyncCollab && AsyncCollab.contribute && asyncCollabState && localPlayer) {
            var buildProjects = AsyncCollab.getPlayerProjects ? AsyncCollab.getPlayerProjects(asyncCollabState, localPlayer.id) : [];
            if (buildProjects.length > 0) {
              AsyncCollab.contribute(asyncCollabState, localPlayer.id, buildProjects[0].id, 3, 'Built ' + result.type, worldTime);
            }
          }

          // PlayerOnboarding: complete build step
          if (PlayerOnboarding && PlayerOnboarding.completeStep && playerOnboardingState) {
            PlayerOnboarding.completeStep(playerOnboardingState, localPlayer.id, 'place_building', worldTime);
          }
      ...
      case 'interact':
        ...
        if (NPCs && NPCs.interactWithNPC && localPlayer) {
          ...
          if (npcResponse) {
            ...
            if (Quests && npcResponse.questInfo) {
              ...
              if (questInfo.state === 'available') {
                ...
                if (result.success && HUD) {
                  ...
                  // SkillMastery: questing XP
                  if (SkillMastery && SkillMastery.addXP && skillMasteryState) {
                    SkillMastery.addXP(skillMasteryState, 'questing', 15);
                  }

                  // MentorGuilds: quest completion counts as lesson progress
                  if (MentorGuilds && MentorGuilds.completeLesson && typeof gameState !== 'undefined' && gameState && gameState.subsystems) {
                    MentorGuilds.completeLesson(gameState.subsystems, localPlayer.id, questInfo.quest.id, worldTime);
                  }

                  // ConsequenceBranching: quest choices may define path
                  if (ConsequenceBranching && ConsequenceBranching.makeDecision && consequenceBranchingState) {
                    var questDecisionId = 'quest_' + questInfo.quest.id;
                    ConsequenceBranching.makeDecision(consequenceBranchingState, localPlayer.id, questDecisionId, 'complete', worldTime);
                  }

                  // MultiverseReputation: earn rep from quest completion
                  if (MultiverseReputation && MultiverseReputation.earnReputation && multiverseRepState) {
                    MultiverseReputation.earnReputation(multiverseRepState, localPlayer.id, 'zion', 5, 'quest_complete');
                  }

                  // PlayerOnboarding: complete quest step
                  if (PlayerOnboarding && PlayerOnboarding.completeStep && playerOnboardingState) {
                    PlayerOnboarding.completeStep(playerOnboardingState, localPlayer.id, 'complete_quest', worldTime);
                  }
            ...
            if (NarrativeThreads && NarrativeThreads.addBeat && narrativeThreadsState) {
              var playerThreads = NarrativeThreads.getActiveThreads ? NarrativeThreads.getActiveThreads(narrativeThreadsState) : [];
              if (playerThreads.length > 0) {
                NarrativeThreads.addBeat(narrativeThreadsState, localPlayer.id, playerThreads[0].id, npcResponse.name + ' shared wisdom', 'npc_talk', currentZone, worldTime);
              }
            }

            // PlayerOnboarding: complete talk_npc step
            if (PlayerOnboarding && PlayerOnboarding.completeStep && playerOnboardingState) {
              PlayerOnboarding.completeStep(playerOnboardingState, localPlayer.id, 'talk_npc', worldTime);
            }

            // MultiverseReputation: earn rep from NPC interaction
            if (MultiverseReputation && MultiverseReputation.earnReputation && multiverseRepState) {
              MultiverseReputation.earnReputation(multiverseRepState, localPlayer.id, 'zion', 1, 'npc_interaction');
            }
  ...
  function startFishing() {
    ...
    HUD.showFishingUI(currentZone, function(result) {
      ...
      if (result && result.success && result.fish) {
        ...
        // AsyncCollab: fishing effort contributes to projects
        if (AsyncCollab && AsyncCollab.contribute && asyncCollabState && localPlayer) {
          var fishProjects = AsyncCollab.getPlayerProjects ? AsyncCollab.getPlayerProjects(asyncCollabState, localPlayer.id) : [];
          if (fishProjects.length > 0) {
            AsyncCollab.contribute(asyncCollabState, localPlayer.id, fishProjects[0].id, 1, 'Caught fish', worldTime);
          }
        }

        // PlayerOnboarding: complete fishing step
        if (PlayerOnboarding && PlayerOnboarding.completeStep && playerOnboardingState) {
          PlayerOnboarding.completeStep(playerOnboardingState, localPlayer.id, 'catch_fish', worldTime);
        }
