(function(exports) {
  ...
  function nexusAmbient() {
    ...
    try {
      ...
      function chime() {
        if (!ctx || !masterGain) return;
        var freqs = [130.81, 164.81, 196.00, 261.63, 329.63];
    ...
    try {
      ...
      function rustle() {
        ...
        if (n) {
          ...
          rf.type = 'lowpass'; rf.frequency.value = 400;
    ...
    try {
      ...
      function page() {
        ...
        if (n) {
          ...
          pf.type = 'lowpass'; pf.frequency.value = 300;
  ...
  function studioAmbient() {
    ...
    try {
      ...
      function chime() {
        if (!ctx || !masterGain) return;
        var o = ctx.createOscillator(), g = ctx.createGain();
        o.type = 'triangle'; o.frequency.value = 130 + Math.random() * 200;
  ...
  function agoraAmbient() {
    ...
    try {
      ...
      function call() {
        if (!ctx || !masterGain) return;
        var scale = [110, 123, 131, 147, 165, 175, 196, 220];
    ...
    try {
      ...
      function crackle() {
        ...
        if (n) {
          ...
          ff.type = 'lowpass'; ff.frequency.value = 300 + Math.random() * 100;
  ...
  function arenaAmbient() {
    ...
    try {
      ...
      if (crowd) {
        ...
        cf.type = 'lowpass'; cf.frequency.value = 400; cg.gain.value = 0.035;
      ...
      function clash() {
        ...
        if (n) {
          ...
          mf.type = 'lowpass'; mf.frequency.value = 300;
  ...
  var SFX = {
    ...
    harvest: function() {
      tone(165, 'triangle', 0.06, 0.3);
      tone(220, 'triangle', 0.03, 0.3);
    },
    build: function() {
      tone(80, 'sine', 0.06, 0.15);
      tone(160, 'sine', 0.04, 0.02);
    },
    trade: function() {
      if (!ctx || !masterGain) return;
    ...
    trade_request: function() { tone(220, 'sine', 0.04, 0.5); },
    discover: function() { arp([130.81, 164.81, 196.00, 261.63], 0.08, 'sine', 0.05, 0.2); },
    challenge: function() {
      if (!ctx || !masterGain) return;
    ...
    levelup: function() { arp([130.81, 164.81, 196.00, 261.63], 0.1, 'sine', 0.05, 0.7); },
    error: function() { tone(80, 'sine', 0.04, 0.2); },
    notification: function() { tone(220, 'sine', 0.05, 0.4); },
    craft_complete: function() {
      tone(120, 'sine', 0.06, 0.2);
      arp([165, 196, 220], 0.05, 'sine', 0.035, 0.4, 0.1);
    },
    quest_accept: function() { arp([130.81, 164.81, 196.00], 0.08, 'sine', 0.05, 0.25); },
    ...
    npc_greet: function() { tone(165, 'sine', 0.04, 0.35); },
    portal_enter: function() {
      ...
      if (n) {
        ...
        f.frequency.exponentialRampToValueAtTime(400, ctx.currentTime + 0.8);
        f.Q.value = 2; g.gain.value = 0.06; g.gain.exponentialRampToValueAtTime(0.001, ctx.currentTime + 0.8);
        n.connect(f); f.connect(g); g.connect(masterGain);
      }
    ...
    build_place: function() { tone(60, 'sine', 0.06, 0.12); },
    footstep: function() {
      if (!ctx || !masterGain) return;
    ...
    shutter: function() {
      ...
      [now, now + 0.035].forEach(function(t) {
        ...
        if (n) {
          ...
          hp.type = 'lowpass'; hp.frequency.value = 300; g.gain.value = 0.04;
  ...
  var FOOTSTEPS = {
    ...
    stone: function() { tone(120, 'sine', 0.04, 0.05); tone(200, 'sine', 0.05, 0.02); },
    sand: function() {
      var n = whiteNoise(0.12);
      if (n) {
        var f = ctx.createBiquadFilter(), g = ctx.createGain();
  ...
  function rustleLayer(s, minGap, maxGap, vol) {
    function rustle() {
      ...
      if (n) {
        var f = ctx.createBiquadFilter(), g = ctx.createGain();
        f.type = 'lowpass'; f.frequency.value = 300 + Math.random() * 100;
  ...
  var TOD_AMBIENTS = {
    ...
    morning: function() {
      ...
      function buzz() {
        if (!ctx || !masterGain) return;
        var o = ctx.createOscillator(), g = ctx.createGain();
        o.type = 'sine'; o.frequency.value = 250 + Math.random() * 100;
  ...
  function updateAmbientForWeather(weather) {
    ...
    var fns = {
      ...
      rain: function() {
        ...
        if (rain) {
          var rf = ctx.createBiquadFilter(), rg = ctx.createGain();
          rf.type = 'lowpass'; rf.frequency.value = 400; rg.gain.value = 0.05;
  ...
  var ZONE_LAYERS = {
    nexus: function() {
      ...
      o1.type = 'sine'; o1.frequency.value = 130;
    ...
    wilds: function() {
      ...
      if (w) {
        var f = ctx.createBiquadFilter(), g = ctx.createGain();
        f.type = 'lowpass'; f.frequency.value = 300; g.gain.value = 0.035;
        w.connect(f); f.connect(g); g.connect(masterGain);
        s.nodes.push(w, f, g);
      }
      return s;
    ...
    commons: function() {
      ...
      function chime() {
        if (!ctx || !masterGain) return;
        var o = ctx.createOscillator(), g = ctx.createGain();
        o.type = 'sine'; o.frequency.value = 150 + Math.random() * 100;
    ...
    arena: function() {
      ...
      if (e) {
        ...
        ef.type = 'lowpass'; ef.frequency.value = 300;
  ...
  var NPC_SOUNDS = {
    hammer: function() { tone(90, 'sine', 0.075, 0.2); tone(160, 'sine', 0.035, 0.3); },
    music: function() { arp([196.00, 220.00, 246.94], 0.15, 'sine', 0.045, 0.25); },
    garden: function() {
  ...
  var SCALES = {
    nexus:     [64.22, 72.08, 80.91, 96.23, 108.00, 128.44, 144.16, 161.81],
    gardens:   [72.08, 80.91, 90.82, 108.00, 121.23, 144.16, 161.81, 181.63],
    athenaeum: [60.61, 68.05, 80.91, 90.82, 101.94, 121.23, 136.09, 161.81],
    studio:    [80.91, 90.82, 101.94, 121.23, 136.09, 161.81, 181.63, 203.88],
    wilds:     [54.00, 64.22, 72.08, 80.91, 96.23, 108.00, 128.44, 144.16],
    agora:     [85.72, 96.23, 108.00, 128.44, 144.16, 171.44, 192.46, 216.00],
    commons:   [72.08, 85.72, 96.23, 108.00, 128.44, 144.16, 171.44, 192.46],
    arena:     [96.22, 114.42, 128.44, 144.16, 171.44, 192.46, 228.85, 256.87]
  };
  ...
  function scheduleMelody() {
    ...
    musicState.melodyTimeout = setTimeout(function() {
      ...
      if (Math.random() < style.melodyChance * tmod.melM) {
        ...
        melFilter.type = 'lowpass'; melFilter.frequency.value = 400;
  ...
  var PIANO_ACCENTS = {
    dawn:          { notes: [65.41, 82.41, 98.00, 130.81, 164.81],                                 noteSpacing: 0.18, noteDuration: 1.2, volume: 0.05 },
    morning:       { notes: [98.00, 123.47, 146.83, 196.00, 246.94, 196.00],                       noteSpacing: 0.15, noteDuration: 1.0, volume: 0.04 },
    dusk:          { notes: [220.00, 164.81, 130.81, 110.00, 82.41],                               noteSpacing: 0.20, noteDuration: 1.4, volume: 0.04 },
    night:         { notes: [73.42, 87.31, 110.00, 98.00, 73.42],                                  noteSpacing: 0.30, noteDuration: 1.8, volume: 0.04 },
    zone_discovery:{ notes: [98.00, 110.00, 130.81, 146.83, 164.81, 196.00, 164.81, 196.00],       noteSpacing: 0.12, noteDuration: 0.9, volume: 0.06 },
    quest_complete:{ notes: [130.81, 164.81, 196.00, 261.63, 196.00, 261.63, 329.63],              noteSpacing: 0.14, noteDuration: 1.0, volume: 0.06 },
    achievement:   { notes: [130.81, 146.83, 164.81, 196.00, 220.00, 261.63],                      noteSpacing: 0.12, noteDuration: 0.8, volume: 0.06 },
    level_up:      { notes: [65.41, 82.41, 98.00, 130.81, 164.81, 196.00, 261.63],                 noteSpacing: 0.14, noteDuration: 1.2, volume: 0.06 }
  };
